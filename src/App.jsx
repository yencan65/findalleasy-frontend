...
useEffect(() => {
  ...
  return (
    <div
      className="min-h-[100dvh] flex flex-col text-white font-sans overflow-x-hidden relative bg-transparent"
    >
      <NeuralBackground />
      <Header />
      <NetworkStatusBanner />
      ...
      <main className="flex-1 flex flex-col items-center justify-start w-full px-4 pt-10 sm:pt-14 pb-28 sm:pb-8 relative z-[5]">
        {/* ◆ SLOGAN */}
        <h2 className="flex flex-wrap justify-center items-center gap-x-2 gap-y-1 text-[18px] sm:text-[20px] md:text-[24px] lg:text-[29px] font-semibold text-center select-none px-2 leading-tight text-white/95 fae-slogan">
          <span className="text-white/90">{t("yazman yeterli,")}</span>
          <span className="text-white/90">{t("gerisini")}</span>
          ...
          >
            <img
              src={sonoFace}
              alt="Sono"
              className="relative w-[24px] h-[24px] sm:w-[26px] sm:h-[26px] rounded-full sono-vite fae-sono-shadow drop-shadow-[0_0_10px_rgba(122,92,255,0.35)]"
            />
          </div>

          <span className="text-white/90">{t("halleder.")}</span>
        </h2>
          ...
          <div className="flex flex-col gap-2 w-full md:flex-row md:items-center md:gap-3">
            ...
            <div className="relative w-full md:flex-1">
              <input
                ...
                className="w-full h-11 sm:h-12 rounded-xl pl-9 sm:pl-11 pr-11 sm:pr-12 text-white placeholder:text-white/45 outline-none border border-[rgba(212,175,55,0.55)] focus:border-[rgba(212,175,55,0.75)] bg-[var(--field-surface)] backdrop-blur shadow-[0_0_18px_rgba(212,175,55,0.14)]"
              />

              <button
                id="search-button"
                onClick={doSearch}
                className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 p-1 rounded-md bg-transparent hover:bg-[#D9A441]/10 active:scale-95 transition"
                aria-label={t("search.search")}
                title={t("search.search")}
              >
                <SearchIcon className="w-4 h-4 sm:w-5 sm:h-5 text-[#D9A441]/90" aria-hidden />
              </button>
              ...
              {String(value || "").trim() ? (
                <button
                  ...
                  className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-md bg-transparent hover:bg-[#D9A441]/10 active:scale-95 transition"
                  aria-label={t("actions.clear", { defaultValue: "Temizle" })}
                  title={t("actions.clear", { defaultValue: "Temizle" })}
                >
                  <X className="w-4 h-4 sm:w-5 sm:h-5 text-white/55" aria-hidden />
                </button>
              ) : null}
            </div>
            ...
            <div className="flex items-center justify-end gap-2">
              <button
                onClick={startMic}
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl border border-[rgba(212,175,55,0.55)] bg-[var(--ui-surface)] hover:bg-[var(--ui-surface-strong)] flex items-center justify-center transition ${voiceListening ? "ring-2 ring-[rgba(212,175,55,0.35)] bg-white/5" : ""}`}
              ...
              <button
                onClick={openCamera}
                className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border border-[rgba(212,175,55,0.55)] bg-[var(--ui-surface)] hover:bg-[var(--ui-surface-strong)] flex items-center justify-center transition"
              ...
              >
                <QrCode className="w-5 h-5 sm:w-[22px] sm:h-[22px] text-[#D9A441]/90" aria-hidden />
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-[760px] -mt-1 mb-2">
          {visionBusy ? (
            <div className="flex items-center justify-center gap-2 text-[12px] sm:text-[13px] text-white/70 select-none">
              <div className="w-3 h-3 rounded-full border border-[rgba(122,92,255,0.85)] border-t-transparent animate-spin" />
            ...
            <div className="flex items-center justify-center gap-2 text-[12px] sm:text-[13px] text-white/70 select-none">
              ...
              <span>{t("search.searching", { defaultValue: "Arama yapılıyor..." })}</span>
				...
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 rounded-xl border border-white/12 bg-[var(--ui-surface)] px-3 py-2 shadow-[0_18px_55px_rgba(122,92,255,0.12)]">
				  <div className="text-[12px] sm:text-[13px] text-white/75">
				    {scanConfirm?.isBarcode ? (
				      <>
				        <span className="text-white/65">{t("search.barcodeDetected", { defaultValue: "Barkod algılandı: {{code}}", code: String(scanConfirm?.code || value || "").trim() })}</span>
				        <span className="text-white/65">{" "}— {t("search.confirmQuestion", { defaultValue: "Bu aramayı yapmak istiyor musunuz?" })}</span>
				      </>
				    ) : (
				      <>
				        <span className="text-white/65">{t("search.voiceHeardPrefix", { defaultValue: "Anladığım:" })}</span>{" "}
				  ...
				  <div className="flex items-center gap-2">
				    ...
				    <button
				      ...
				      className="text-xs px-3 py-1 rounded-lg border border-white/20 text-[#D9A441]/90 hover:bg-[#D9A441]/10"
				    >
				    ...
				    <button
				      ...
				      className="text-xs px-3 py-1 rounded-lg border border-white/15 text-white/60 hover:bg-[#D9A441]/10"
				    >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 rounded-xl border border-white/12 bg-[var(--ui-surface)] px-3 py-2 shadow-[0_18px_55px_rgba(122,92,255,0.12)]">
              <div className="text-[12px] sm:text-[13px] text-white/75">
                <span className="text-white/65">
                  {visionConfirm?.weak
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg border border-[#D9A441]/45 text-white text-[12px] hover:bg-[#D9A441]/10 active:scale-95 transition"
                >
                  {t("search.editQuery", { defaultValue: "Düzenle" })}
                </button>
                <button
                  onClick={() => setVisionConfirm(null)}
                  className="px-3 py-1.5 rounded-lg border border-white/20 text-[#D9A441]/90 text-[12px] hover:bg-[#D9A441]/10 active:scale-95 transition"
                >
...
{/* ◆ Akıllı Selam */}
  ...
  <p className="text-[13px] sm:text-[14px] text-[#D9A441]/90 text-center flex items-center gap-2">
    <span>{greeting}</span>
