...
export default function Vitrin() {
  ...
  // ============================================================
  // ✅ Inject event: barcode/product-info gibi dış hatlardan vitrine direkt veri bas
  // ============================================================
  useEffect(() => {
    const onInject = (e) => {
      const d = e?.detail || {};
      ...
      // En güvenli/ucuz en öne (Vitrin sadece best[0] gösteriyor)
      const items = d.items
        .filter(Boolean)
        .slice()
        .sort((a, b) => {
          ...
          return (pa == null ? 9e15 : pa) - (pb == null ? 9e15 : pb);
        });
      ...
      if (q) {
        setLastQuery(q);
        try {
          if (typeof window !== "undefined" && window.localStorage) window.localStorage.setItem("lastQuery", q);
        } catch {}
      }

      setBest(items.length ? [items[0]] : []);
      setSmart(items.length > 1 ? items.slice(1) : []);
      setOthers([]);
